...
14. **Comments are commit-scoped via `Comment::commit_id`.** When the inline commit selector shows exactly one commit, `App::save_comment` stamps that commit's SHA on the comment. Comments with `commit_id = Some(sha)` are hidden when a different commit (or a subset not containing `sha`) is selected; `commit_id = None` (legacy, review-level, or made against the full cumulative diff) is always visible. The filter runs in `rebuild_annotations`, both diff renderers, the comment navigator (via filtered annotations), and the submit preflight. `App::comment_visible(&Comment)` is the single predicate — it folds in the settled-thread filter too (`thread_dropped`), so a caller cannot apply one and forget the other. Loops that already hold a commit set call `comment_visible_in(&Comment, set)`; `comment_visible_with` is the commit half alone and is only correct beside an explicit settled-thread check. `App::resolved_threads` (`ResolvedThreadsVisibility::{Expanded, Collapsed, Hidden}`, `:threads resolved` / `open` / `hide`, `<leader>R` cycles) decides whether a settled thread renders in full, as a one-line marker, or not at all; `CommentLayout` in `src/app/annotations.rs` mirrors those predicates because `rebuild_annotations` cannot reborrow `self` while pushing rows — the two must not drift. `AnnotatedLine::LineComment`/`FileComment` `comment_idx` is the **absolute** index into the stored `Vec`/`HashMap` value — `delete_comment_at_cursor` and `enter_edit_mode` must look it up directly, not re-count by side.
